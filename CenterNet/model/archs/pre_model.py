...
class Cascade_corner_pooling(nn.Module):
    def __init__(self, dim, stance1, stance2):
        self.p_conv1 = convolution(dim, dim, 3)
        self.p_conv2 = convolution(dim, dim, 3)

        self.corner_pooling1 = Corner_pooling(stance1)
        self.corner_pooling2 = Corner_pooling(stance2)

        self.conv = nn.Conv2d(dim, dim, kernel_size=3, bias=False)

    def forward(self, x):
        p_conv1 = self.p_conv1(x)
        p_conv2 = self.p_conv2(x)

        corner_pooling1 = self.corner_pooling1(p_conv1)
        conv = self.conv(p_conv2 + corner_pooling1)
        corner_pooling2 = self.corner_pooling2(conv)

        return corner_pooling2


class Center_pooling_module(nn.Module):
    def __init__(self, dim):
        self.p_conv1 = convolution(dim, dim, 3)
        self.p_conv2 = convolution(dim, dim, 3)

        self.left_pooling = Corner_pooling('left')
        self.right_pooling = Corner_pooling('right')
        self.top_pooling = Corner_pooling('top')
        self.bottom_pooling = Corner_pooling('bottom')

    def forward(self, x):
        p_conv1 = self.p_conv1(x)
        p_conv2 = self.p_conv2(x)

        left = self.left_pooling(p_conv1)
        right = self.right_pooling(left)
        top = self.top_pooling(p_conv2)
        bottom = self.bottom_pooling(top)
